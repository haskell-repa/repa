...
static inline long distance
	( long x0, long y0		// point
	, long x1, long y1		// a point on the line
	, long x2, long y2)		// another point on the line
{
	...
	, long  x2, 		long y2)
{
	...
	for (int i = 0; i < points->length; i++) {
		long px	= points->x[i];
		long py	= points->y[i];
		long d 	= distance(px, py, x1, y1, x2, y2);
	...
	, Vector* hull)		// Buffer to write the hull points to.
{
	...
	long leftX	= points->x[0];
	long leftY	= points->y[0];
	long rightX	= points->x[0];
	long rightY	= points->y[0];
	for(int i = 0; i < points->length; i++) {
		long px	= points->x[i];
		long py	= points->y[i];

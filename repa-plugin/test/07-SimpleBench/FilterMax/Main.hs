...
main
 = do   args    <- getArgs
        ...
        ((uvec', n1), t) 
         <- R.time $ do (uvec', n1) <- run alg uvec
                        uvec' `seq` n1 `seq` return (uvec', n1)
...
-------------------------------------------------------------------------------
run_vector :: U.Vector Int  -> IO (U.Vector Int, Int)
run_vector uvec1
 = let  uvec2   = U.map (+ 1) uvec1
        uvec4   = U.filter (\x -> x > 0) uvec2
        n1      = U.foldl maxx 0 uvec4
   in   return  (uvec4, n1)
{-# NOINLINE run_vector #-}


run_flow  :: U.Vector Int -> IO (U.Vector Int, Int)
run_flow uvec
 = do   vvec2   <- V.fromUnboxed uvec
        let (vvec3, n1) = R.runSeries vvec2 lower_maxx

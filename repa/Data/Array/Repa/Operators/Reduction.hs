...
--
foldS   :: (Source r sh a, Source r (sh :. Int) a, Elt a, Unbox a)
        => (a -> a -> a)
        -> a
        -> Array r (sh :. Int) a
        -> Array U sh a
...
--   AUnboxed (Z :. 2) (fromList [2,4])
--
foldP   :: (Source r sh a, Source r (sh :. Int) a, Elt a, Unbox a, Monad m)
        => (a -> a -> a)
        -> a
        -> Array r (sh :. Int) a
        -> m (Array U sh a)
...
sumS    :: (Source r sh a, Source r (sh :. Int) a, Num a, Elt a, Unbox a)
        => Array r (sh :. Int) a
        -> Array U sh a
sumS = foldS (+) 0
{-# INLINE [3] sumS #-}
...
sumP    :: (Source r sh a, Source r (sh :. Int) a, Num a, Elt a, Unbox a, Monad m)
        => Array r (sh :. Int) a
        -> m (Array U sh a)
sumP = foldP (+) 0 
{-# INLINE [3] sumP #-}
...
-- Equality ------------------------------------------------------------------
instance (Eq sh, Source r sh a, Eq a) => Eq (Array r sh a) where
 (==) arr1 arr2
...
--   in parallel.
equalsP :: (Eq sh, Source r1 sh a, Source r2 sh a, Eq a, Monad m) 
        => Array r1 sh a 
        -> Array r2 sh a
        -> m Bool
equalsP arr1 arr2
...
-- | Check whether two arrays have the same shape and contain equal elements,
--   sequentially.
equalsS :: (Eq sh, Source r1 sh a, Source r2 sh a, Eq a) 
        => Array r1 sh a 
        -> Array r2 sh a
        -> Bool
equalsS arr1 arr2

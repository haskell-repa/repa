...
module Data.Array.Repa.Arbitrary( arbitraryUShaped
                                , forAllUShaped
                                , forAll2UShaped
                                , forAll3UShaped
                                , forAll4UShaped
                                ...
                                , forAll4VShaped
                                , forAll5VShaped
                                ) -- we also export instances of Arbitrary and CoArbitrary
where
...
-- Arbitrary coming here
instance Arbitrary Z where
  arbitrary = return Z

-- Note: this is a shape that is "sized", and then random array for a given shape is generated.
instance (Shape a, Arbitrary a) => Arbitrary (a :. Int) where
   arbitrary = sized (\n ->do b <- choose (1, n)
                              ...
                              return $ a :. b) -- each dimension should be at least 1-wide
...
instance (Arbitrary sh, Arbitrary a, U.Unbox a, Shape sh) => Arbitrary (Array U sh a) where
  arbitrary = arbitrary >>= arbitraryUShaped

instance (Arbitrary sh, Arbitrary a, Shape sh) => Arbitrary (Array V.V sh a) where
  arbitrary = arbitrary >>= arbitraryVShaped

-- Coarbitrary coming here...
instance CoArbitrary Z where
  coarbitrary _ = id 

instance (Shape a, CoArbitrary a) => CoArbitrary (a :. Int) where
  coarbitrary (a :. b) = coarbitrary a >< coarbitrary b

instance (CoArbitrary sh, CoArbitrary a, Source r a,
          Shape sh) => CoArbitrary (Array r sh a) where
  coarbitrary arr = (coarbitrary . extent $ arr) >< (coarbitrary . toList $ arr)

{- Since there is an 'instance @Source@ D a' leaving this unresolved is probably a bug on part of GHC.
instance (Arbitrary sh, Arbitrary a, U.Unbox a, Shape sh) => Arbitrary (Array D sh a) where
  arbitrary = delay `fmap` arbitrary   
 -}

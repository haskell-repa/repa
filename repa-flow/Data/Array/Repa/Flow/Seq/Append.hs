 ...
 where
        ...
        start
         ...
         = do   state <- unew 6
                ...
                iwrite state sSegOff     0#
                iwrite state sIndexA     0#
                iwrite state sIndexB     0#
                iwrite state sNextSwap   0#
         ...
         = do   state   <- unew 6
                iwrite state sTakeFrom  0#
                iwrite state sSegOff    seg_off
                iwrite state sIndexA    (segIdxA seg_off +# el_off)
                iwrite state sIndexB    (segIdxB seg_off)
                iwrite state sNextSwap  (segLenA seg_off -# el_off)
                iwrite state sRemain    n
                return state
        ...
        -- Start reading elements from the B vector.
         ...
         = do   state   <- unew 6
                ...
                iwrite state sTakeFrom  1#
                iwrite state sSegOff    seg_off
                iwrite state sIndexA    (segIdxA seg_off +# segLenA seg_off)
                iwrite state sIndexB    (segIdxB seg_off +# el_off')
                iwrite state sNextSwap  (segLenB seg_off -# el_off')
                iwrite state sRemain    n
                return state

        size state
         = do   !(I# remain)  <- iread state sRemain 
                return $ Exact remain
         ...
         = do   !(I# nextSwap)  <- iread state sNextSwap
                ...
                if nextSwap ==# 0#
                 ...
                 -- Emit a value from the A vector.
                        ...
                        !(I# indexA)    <- iread state sIndexA
                        !(I# remain)    <- iread state sRemain
                        iwrite state sIndexA   (indexA   +# 1#)
                        iwrite state sNextSwap (nextSwap -# 1#)
         ...
         = do   !(I# nextSwap)  <- iread state sNextSwap
                ...
                if nextSwap ==# 0#
                 ...
                 -- Emit a value from the B vector.
                        ...
                        !(I# indexB)    <- iread state sIndexB
                        !(I# remain)    <- iread state sRemain
                        iwrite state sIndexB   (indexB   +# 1#)
                        iwrite state sNextSwap (nextSwap -# 1#)
                        iwrite state sRemain   (remain   -# 1#)
